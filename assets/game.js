(() => {
  ...
  const VERSION = 3;
  const SAVE_KEY = 'mianmu-the-face-of-it-save-v3';
  const META_KEY = 'mianmu-the-face-of-it-meta-v3';
  ...
  function sceneHTML(){
    ...
    return `<div class="scene-frame ${s.cls}"><div class="scene-bg" style="background-image:url('assets/images/${s.img}')"></div><div class="scene-vignette"></div><div class="scene-overlays">${sceneHotspots(scene)}</div><div class="scene-caption"><strong>${s.title}</strong>${s.caption}</div></div>`;
  }
  ...
  function openPostman(){
    ...
    modal(`<div class="kicker">邮差 · 埃利亚斯</div><h2>第七步以前，一切都像昨天</h2><p class="lead">六个邮箱都只说“昨天也这样”。没有隐藏密码；真正的规则是你的脚步。</p><div class="puzzle-visual" style="min-height:520px;background-image:url('assets/images/postman.webp')"><div class="route-ui"><button class="route-btn" id="route-back">向后一步</button><div><div class="route-progress" id="route-progress">${Array.from({length:7},()=>'<div class="step"></div>').join('')}</div><div class="route-status" id="route-status">起点。海边的第七封信不在地图上。</div></div><button class="route-btn" id="route-forward">向前一步</button></div></div><div id="post-feedback" class="feedback" style="margin-top:10px">规则会先重复给你看，而不是只出现一次就要求猜答案。</div>`,{dark:true,wide:true,close:true});
